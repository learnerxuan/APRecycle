...
if ($challenge_id > 0) {
    ...
    <div class="page-header" style="margin-bottom: var(--space-6);">
        <div style="display: flex; justify-content: space-between; align-items: center;">
            <div>
                <a href="leaderboard_challenges.php"
                    style="display: inline-flex; align-items: center; gap: 8px; padding: 8px 16px; background: white; color: var(--color-gray-700); text-decoration: none; border-radius: 8px; border: 1px solid var(--color-gray-200); font-weight: 500; transition: all 0.2s; box-shadow: 0 1px 2px rgba(0,0,0,0.05); margin-bottom: var(--space-4);"
            ...
            <div class="badge"
                style="background: <?php echo $challenge_info['challenge_status'] == 'Active' ? 'var(--color-success-light)' : 'var(--color-gray-200)'; ?>; color: <?php echo $challenge_info['challenge_status'] == 'Active' ? '#065F46' : 'var(--color-gray-700)'; ?>; font-size: 1rem; padding: 8px 16px; font-weight: 600; border-radius: 20px;">
    ...
    <?php if (count($rankings) > 0): ?>
        <div
            style="display: grid; grid-template-columns: repeat(3, 1fr); gap: var(--space-6); margin-bottom: var(--space-10); align-items: end;">
            <?php
            $podium = [1 => '🥈', 0 => '🥇', 2 => '🥉'];
            $order = [1, 0, 2];
            foreach ($order as $idx):
                if (isset($rankings[$idx])):
                    ...
                    <div class="card text-center"
                        style="padding: var(--space-6); border-bottom: 4px solid <?php echo $is_gold ? '#FBBF24' : 'var(--color-gray-300)'; ?>; background: white; position: relative; overflow: hidden;">
                        <?php if ($is_gold): ?>
                            <div
                                style="position: absolute; top: 0; left: 0; width: 100%; height: 6px; background: linear-gradient(90deg, #FCD34D, #F59E0B);">
                            </div><?php endif; ?>
                        <div style="font-size: 3rem; margin-bottom: var(--space-2);"><?php echo $podium[$idx]; ?></div>
                        <div
                            style="width: 60px; height: 60px; background: var(--gradient-primary); color: white; border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto var(--space-3); font-weight: bold; font-size: 1.25rem;">
                            <?php echo strtoupper(substr($u['username'], 0, 1)); ?>
                        </div>
                        <h3 style="margin-bottom: var(--space-1);"><?php echo htmlspecialchars($u['username']); ?></h3>
                        <p style="color: var(--color-primary); font-weight: 700; font-size: 1.25rem;">
                            <?php echo number_format($u['challenge_point']); ?> pts
                        </p>
                    </div>
            ...
            <table style="width: 100%; border-collapse: collapse;">
                ...
                <tbody>
                    <?php for ($i = 3; $i < count($rankings); $i++):
                        ...
                        ?>
                        <tr
                            style="border-bottom: 1px solid var(--color-gray-100); <?php echo $is_me ? 'background: var(--color-success-light);' : ''; ?>">
                            <td style="padding: var(--space-4); font-weight: 700; color: var(--color-gray-500);">
                                #<?php echo $i + 1; ?></td>
                            <td style="padding: var(--space-4); font-weight: 500;">
                                <?php echo htmlspecialchars($row['username']); ?>
                                <?php if ($is_me): ?><span class="badge"
                                        style="background: var(--color-primary); color: white; font-size: 0.7rem; padding: 2px 6px; margin-left: 8px;">YOU</span><?php endif; ?>
                            </td>
                    ...
                    <?php if (count($rankings) <= 3 && count($rankings) > 0): ?>
                        <tr>
                            <td colspan="3" style="padding: var(--space-8); text-align: center; color: var(--color-gray-500);">Top
                                recyclers are shown above!</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
    ...
    <?php if (count($active_challenges) > 0):
        ...
        <div class="hero-challenge">
            <div class="hero-content">
                ...
                <div>
                    <a href="?id=<?php echo $hero['challenge_id']; ?>" class="btn"
                        style="background: white; color: var(--color-primary); padding: 12px 32px; font-weight: 700; border: none; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
            ...
            <?php for ($i = 1; $i < count($active_challenges); $i++):
                ...
                <div class="challenge-card">
                    <div class="challenge-header">
                        ...
                        <span
                            style="font-size: 0.8rem; font-weight: 700; color: #059669; background: #D1FAE5; padding: 2px 8px; border-radius: 4px;">ACTIVE</span>
                    </div>
                    <p style="color: var(--color-gray-600); font-size: 0.9rem; flex-grow: 1;">
                        <?php echo htmlspecialchars(substr($c['description'], 0, 80)) . '...'; ?>
                    </p>

                    <div
                        style="border-top: 1px solid var(--color-gray-100); margin-top: var(--space-4); padding-top: var(--space-4);">
                        <div class="meta-row">
                    ...
                    <a href="?id=<?php echo $c['challenge_id']; ?>" class="btn-action btn-primary"
    ...
    <?php if (mysqli_num_rows($upcoming_result) > 0): ?>
        <h2
            style="margin: var(--space-8) 0 var(--space-4); color: var(--color-gray-800); border-top: 1px solid var(--color-gray-200); padding-top: var(--space-8);">
            ...
            <?php while ($c = mysqli_fetch_assoc($upcoming_result)): ?>
                <div class="challenge-card" style="opacity: 0.8; background: var(--color-gray-50);">
                    <div class="challenge-header">
                        <h3 class="challenge-title"><?php echo htmlspecialchars($c['title']); ?></h3>
                        <span
                            style="font-size: 0.75rem; font-weight: 700; color: #1E40AF; background: #DBEAFE; padding: 2px 8px; border-radius: 4px;">SOON</span>
                    </div>
                    <div class="meta-row">
                        <i class="far fa-calendar-alt"></i> Starts <?php echo date('M d', strtotime($c['start_date'])); ?>
                    </div>
                    <p style="color: var(--color-gray-600); font-size: 0.9rem; margin-top: var(--space-2);">
    ...
    <?php if (mysqli_num_rows($past_result) > 0): ?>
        ...
        <div class="challenges-grid">
            <?php while ($c = mysqli_fetch_assoc($past_result)): ?>
                <div class="challenge-card">
                    <div class="challenge-header">
                        <h3 class="challenge-title" style="color: var(--color-gray-600);">
                            <?php echo htmlspecialchars($c['title']); ?>
                        </h3>
                        <span
                            style="font-size: 0.75rem; font-weight: 700; color: var(--color-gray-600); background: var(--color-gray-200); padding: 2px 8px; border-radius: 4px;">ENDED</span>
                    </div>
                    <div class="meta-row">
                        <i class="fas fa-flag-checkered" style="color: var(--color-gray-400);"></i> Ended
                        <?php echo date('M d', strtotime($c['end_date'])); ?>
                    </div>
                    <?php if ($c['my_points'] > 0): ?>
                    ...
                    <?php endif; ?>
                    <a href="?id=<?php echo $c['challenge_id']; ?>" class="btn-action btn-outline"
                        style="margin-top: var(--space-4);">
                        See Winners
                    </a>
                </div>
